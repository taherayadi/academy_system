...
export async function readStaff(db: D1Database): Promise<any[]> {
  ...
  return staffRows.results.map((r: any) => ({ id: str(r.id), firstName: str(r.first_name), lastName: str(r.last_name), cin: str(r.cin), cnssNumber: str(r.cnss_number), subjects: subjectsByStaff[str(r.id)] || [], salary: num(r.salary), type: str(r.type), phone: str(r.phone), role: str(r.role), contractStartDate: str(r.contract_start_date), contractType: r.contract_type == null ? undefined : str(r.contract_type), email: r.email == null ? undefined : str(r.email), address: r.address == null ? undefined : str(r.address), baseSalary: r.base_salary == null ? undefined : num(r.base_salary), cnssAmount: r.cnss_amount == null ? undefined : num(r.cnss_amount), hourlyRate: r.hourly_rate == null ? undefined : num(r.hourly_rate), hireDate: r.hire_date == null ? undefined : str(r.hire_date), leaveRequests: leaveByStaff[str(r.id)] || [], advances: advanceByStaff[str(r.id)] || [], payments: paymentsByStaff[str(r.id)] || [], payslips: payslipsByStaff[str(r.id)] || [], schedule: scheduleByStaff[str(r.id)] || [] }));
}

function buildStaffStmts(db: D1Database, staff: any[]): D1PreparedStatement[] {
  const stmts: D1PreparedStatement[] = [];
  for (const s of staff || []) {
    stmts.push(db.prepare('INSERT INTO staff (id, first_name, last_name, cin, cnss_number, salary, type, phone, role, contract_start_date, contract_type, email, address, base_salary, cnss_amount, hourly_rate, hire_date) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)').bind(s.id, s.firstName, s.lastName, s.cin, s.cnssNumber, num(s.salary), s.type, s.phone, s.role, s.contractStartDate, s.contractType ?? null, s.email ?? null, s.address ?? null, s.baseSalary ?? null, s.cnssAmount ?? null, s.hourlyRate ?? null, s.hireDate ?? null));
